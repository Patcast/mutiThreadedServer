...
FLAGS = -std=c11  -DTIMEOUT=8 -Werror -lm $(shell pkg-config --cflags --libs check)

test: connmgr.c main.c 
	@echo -e '\n*******************************'
	@echo -e '*** Compiling for UNIT TEST ***'
	@echo -e '*******************************'
	@mkdir -p build
	@gcc connmgr.c main.c  lib/tcpsock.c -lmylist -L/lib/ -o build/test $(FLAGS)
	@echo -e '\n*************************'
